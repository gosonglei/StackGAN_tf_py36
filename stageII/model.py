...
class CondGAN(object):
    def __init__(self, lr_imsize, hr_lr_ratio):
        ...
        self.s = lr_imsize  # 256 / 4 = 64
        print('lr_imsize: ', lr_imsize)
        # s2=64/2 s4=64/4 ....32 16 8 4
    ...
    def generate_condition(self, c_var):
        ...
        # 取前128个数,均值，(batch_size, 128)
        mean = conditions[:, :self.ef_dim]
        # 取后128个数，标准差，(batch_size, 128)
    ...
    # stage II generator (hr_g)   hr高分辨率
    # 残差块
    ...
    def hr_g_joint_img_text(self, x_c_code):
        output_tensor = \
            (pt.wrap(x_c_code).  # -->s4 * s4 * (ef_dim+gf_dim*4) 16*16*（128+128*4）
    ...
    def hr_generator(self, x_c_code):
        output_tensor = \
            (pt.wrap(x_c_code).  # -->s4 * s4 * gf_dim*4
             ...
             conv_batch_norm().
             apply(tf.nn.relu).
             custom_conv2d(3, k_h=3, k_w=3, d_h=1, d_w=1).  # -->4s * 4s * 3  256*256*3
             apply(tf.nn.tanh))
        return output_tensor
